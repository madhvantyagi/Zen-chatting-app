...
export default function Signup({ setUser }) {
  ...
  const handleSignup = async (event) => {
    ...
    const createdUser = await axios.post(URL, {
      email: email,
      name: name,
      username: username,
      password: password,
    });
    setUser(createdUser.data.username);
    localStorage.setItem(createdUser.data.email, createdUser.data.username);
    // console.log(createdUser.data.username);
